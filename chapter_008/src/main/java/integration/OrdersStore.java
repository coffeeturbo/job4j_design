...
public class OrdersStore {
    ...
    public Order save(Order order) {
        return order.getId() == null ? create(order) : update(order);
    }

    private Order update(Order order) {
        try (Connection con = pool.getConnection();
             PreparedStatement pr = con.prepareStatement("UPDATE orders SET name = ?, description = ?, created = ? WHERE id = ?")) {
    ...
    public Order findByName(String name) {
        Order rsl = null;
        try (Connection con = pool.getConnection();
             PreparedStatement pr = con.prepareStatement("SELECT * FROM orders o WHERE o.name = ?")) {
            pr.setString(1, name);
            ResultSet rs = pr.executeQuery();
            if (rs.next()) {
                rsl = new Order(
